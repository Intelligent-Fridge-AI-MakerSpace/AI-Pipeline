...
class ModelProcessor(Node):
    def __init__(self):
        ...
        if self.get_parameter('model_name').get_parameter_value().string_value == 'yolo':
            self.model = a.YoloModel(weights='weights/best.pt', conf=self.get_parameter('conf').get_parameter_value().double_value, iou=self.get_parameter('iou').get_parameter_value().double_value, use_gpu=self.get_parameter('use_gpu').get_parameter_value().bool_value)
        else:
            self.model = a.BarcodeReader()
        self.trackers = {}
    ...
    def callback(self, camera_message: Camera):
        ...
        if camera_message.status in ['START', 'CONTINUE', 'END']:
            ...
            result_message.tags = [c for _, _, _, _, _, c, _ in results]
            ...
            image = self.model.draw(image, results)
        stream_message = self.convertor.cv2_to_imgmsg(image)
